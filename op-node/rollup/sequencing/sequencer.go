...
func NewSequencer(driverCtx context.Context, log log.Logger, rollupCfg *rollup.Config,
	...
	asyncGossip AsyncGossiper,
	metrics SequencerMetrics,
	emitter event.Emitter) *Sequencer {
	return &Sequencer{
		ctx:              driverCtx,
		log:              log,
		rollupCfg:        rollupCfg,
...
func (d *Sequencer) onPendingSafeUpdate(x engine.PendingSafeUpdateEvent) {
	// TODO
	//if x.Unsafe != x.PendingSafe { // if ahead of pending block then we won't have duplicate work.
	//	return
	//}
	//// If we are forcing new blocks onto the tip of the chain, don't try to build on top of it immediately.
	//onto := x.Unsafe
