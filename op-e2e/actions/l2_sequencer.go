...
func NewL2Sequencer(t Testing, log log.Logger, l1 derive.L1Fetcher, blobSrc derive.L1BlobsFetcher,
	...
	var seq *sequencing.Sequencer
	event.Setup(ver.eventSys, "sequencer", event.DefaultRegisterOpts(), func(em event.Emitter) event.Deriver {
		seq = sequencing.NewSequencer(t.Ctx(), log, cfg, attrBuilder, l1OriginSelector,
			seqStateListener, conduc, asyncGossip, metr, ver.synchronousEvents)
		return seq
	})
